...
export interface DialogProps {
  open: boolean;
  dialogClose: () => void;
  handleChangerAdd: (changer: Changer) => void;
}

class ChangerAddDialog extends React.Component<DialogProps, object> {
  constructor(props: DialogProps) {
    super(props);
  }

  render() {
    const { open, dialogClose, handleChangerAdd } = this.props;
    const initialValues: Changer = {
      emoji: '',
      regex: '',
    };
    ...
    return (
      <Dialog open={open} onClose={dialogClose} aria-labelledby="form-dialog-title">
        <Formik
          ...
          onSubmit={values => handleChangerAdd(values)}
        >
          {props => {
            const {
              values,
...
export default ChangerAddDialog;
