...
export interface DialogProps {
  ...
  group: RuleGroup;
  dialogClose: () => void;
  handleRuleEdit: (newRule: RuleGroup) => void;
}

class RuleEditDialog extends React.Component<DialogProps, object> {
  constructor(props: DialogProps) {
    super(props);
  }

  render() {
    const { open, group, dialogClose, handleRuleEdit } = this.props;
    const initialValues: RuleGroup = group;
    ...
    return (
      <Dialog open={open} onClose={dialogClose} aria-labelledby="form-dialog-title">
        <Formik
          ...
          onSubmit={values => handleRuleEdit(values)}
        >
          {props => {
            const {
              values,
